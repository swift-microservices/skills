...
package struct CreateNoteUseCase<DatabaseType>: CreateNoteUseCaseProtocol where DatabaseType: Database, DatabaseType.Scope: CreateNoteUseCaseScope {
    ...
    package func callAsFunction(subject: UserIdentity, input: CreateNoteUseCaseInput) async throws(CreateNoteUseCaseError) -> Note {
        ...
        do {
            let note = try await database.withTransaction { scope in
                let command = CreateNoteCommand(userId: subject.userId, body: input.body)
                return try await scope.noteRepository.create(command)
            }

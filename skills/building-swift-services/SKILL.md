...
32. One routing mechanism and one contract: keep `openapi.yaml` beside `openapi-generator-config.yaml` in the HTTP target that serves it, with `package` access. Default: generate `types` only and register routes by hand on the router, because the router is where the middleware tiers live. Alternative: generated server stubs mounted on the router, when a team wants the compiler to hold the document and the handlers together, at the cost of tiering by middleware instead of by group. A monolith has one document per `<Module>HTTP`, which is that module's contract.
...
41. Apply `BearerAuthenticationInterceptor` then `UserSettingsInterceptor` to the user service, `CertificateAuthenticationInterceptor(authenticator: ServiceAuthenticator())` to the internal service, nothing to the public one, each with `.apply(_, to: .services([descriptor]))`.
...
- The role migrations precede every table; serving connects as the service and internal roles, only the migration client as the owner, and no role has `BYPASSRLS`; every module's migrations are registered through its `<Module>Migrations` list in order.

...
- Concurrency is resolved rather than silenced: no `@unchecked Sendable` without a comment saying what makes it safe, no semaphore or ad-hoc lock inside an async context, no detached task that nothing owns, and cancellation honoured in every worker loop and stream. Evidence: `grep -rn "@unchecked Sendable\|DispatchSemaphore\|Task.detached" Sources`. Judging an isolation question is the `swift-concurrency` skill's ([AvdLee/Swift-Concurrency-Agent-Skill](https://github.com/AvdLee/Swift-Concurrency-Agent-Skill)); load it rather than ruling from the diagnostic.
